<template>
  ...
  <el-button class="focus:outline-none" :loading="true" @click="elClick"
  ...
  <template v-if="defer(3)">
    <button
      @click="testClick"
      type="button"
      class="inline-flex items-center px-4 py-2 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-red-600 hover:bg-red-500 focus:border-red-700 active:bg-red-700 transition ease-in-out duration-150"
      :class="butDis ? 'cursor-not-allowed' : 'cursor-pointer'"
      :disabled="butDis"
    >
      <svg
        v-show="butDis"
...
DefaultData.GetMinuteOption = function () {
  var option = {
    ...
    Type: '分钟走势图', //创建图形类型
    //窗口指标
    Windows: [{ Index: 'MACD', Modify: true, Change: false }],
...
export default defineComponent({
  setup() {
    ...
    const testClick = () => {
      butDis.value = true
      setTimeout(() => (butDis.value = false), 5000)
    }
    return {
      CreateDrawPciture,
...
<style scoped> 
/* .test-but {
  @apply inline-flex items-center px-4 py-2 border border-transparent text-base leading-6 font-medium rounded-md text-white bg-red-600 hover:bg-red-500 focus:border-red-700 active:bg-red-700 transition ease-in-out duration-150;
